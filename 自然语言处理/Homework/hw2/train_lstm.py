...
model = LSTMModel(vocab_size=vocab_size, embeds_size=embeds_size, hidden_size=hidden_size, num_layers=num_layers,
                  dropout=dropout)
model.to(device)
            ...
            if step % 1000 == 0:
                ...
                with torch.no_grad():
                    for batch_valid in valid_dataloader:
                        ...
                        output_valid, h_valid = model(x_valid)
                ...
                ppl_valid = round(np.exp(valid_loss / valid_step), 3)
...
with torch.no_grad():
    for batch_test in test_dataloader:
        ...
        output_test, h_test = model(x_test)
    ...
    ppl_test = round(np.exp(test_loss / test_step), 3)
